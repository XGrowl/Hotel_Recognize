...
public class WaveView extends View {
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        mCircleColor = a.getColor(R.styleable.WaveView_CircleBackgroundColor, Color.parseColor("#F8F8FF"));
        mOutCircleColor = a.getColor(R.styleable.WaveView_outBackgroundColor, Color.parseColor("#F0FFFF"));
        mWaveColor = a.getColor(R.styleable.WaveView_WaveColor, Color.parseColor("#4dd4ff"));
        mTextSise = a.getDimensionPixelSize(R.styleable.WaveView_MidTextSize, 26);
        mBoldTextSize = a.getDimensionPixelSize(R.styleable.WaveView_MidBoldTextSize, 50);
        a.recycle();
        ...
        mBoldTextPaint.setStrokeWidth(5);
        mBoldTextPaint.setColor(Color.BLACK);
        mBoldTextPaint.setTextSize(mBoldTextSize);
        mBoldTextPaint.setTextAlign(Paint.Align.CENTER);
        mBoldTextPaint.setStyle(Paint.Style.FILL);
        mBoldTextPaint.setAntiAlias(true);
        BoldtextHeight = (int) (mBoldTextPaint.descent() - mBoldTextPaint.ascent());
    ...
    private void drawCanvas(Canvas canvas) {
        //画背景圆圈
        canvas.drawCircle(mCentrePoint.x, mCentrePoint.y, mRadius / 0.99f, mOutCirclePaint);
        ...
        if (mStart) {
            ...
            //绘制文字
            if (score == 0) {
                canvas.drawText("--", mCentrePoint.x, mCentrePoint.y - textHeight / 2, mTextPaint);
            } else {
                canvas.drawText(String.valueOf(score),mCentrePoint.x-BoldtextHeight,mCentrePoint.y,mBoldTextPaint);
                canvas.drawText("您已超過"+flowNum + "%"+"的人", mCentrePoint.x, mCentrePoint.y +textHeight *2, mTextPaint);
            }
    ...
    {
        score = num;
        mWaterLevel = (int) (2 * mRadius * score / 1000f);//算出目标水位高度
        hasSetNum = true;
    }
    ...
    public void setBoldTextPaint(float sb) {
        mBoldTextSize = sb;
        mBoldTextPaint.setTextSize(sb);
    }
    //设置水波起伏速度
